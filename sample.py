...
def main(args):
    ...
    #model = torch.jit.load("checkpoints/hugohrban/progen2-small-finetuned/e5/model.safetensors")
    #model = torch.load("model_epoch_1.pth", map_location=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="How many sequences to generate at one iteration. Default: 8",
    )
    parser.add_argument(
        "--max_length",
        type=int,
        default=512,
        help="Maximum length of the generated sequence. Default: 512",
    )
    parser.add_argument(
        "--k",
        type=int,
        default=15,
        help="Top-k sampling parameter. 0 means no top-k sampling. Default: 10",
    )
    parser.add_argument(
